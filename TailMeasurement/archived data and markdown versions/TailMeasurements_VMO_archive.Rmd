...
## Figure 6AA: Plot tail measurement data for vivo-morphants
### plot 72hpa for vmo last batch 022120
```{r plot 1,  echo=FALSE, warning=FALSE, cache=TRUE}

# filter data for last clutch with correct doses
vmo_022120 <- filter(MOdata, Clutch == "22120" )
vmo_022120 <- filter(vmo_022120, SampleID %in% c("tracer", "pbx3_VMO", "meis1_VMO"))
...
ggplot(TailS, aes(x=SampleID, y=Measurement, fill=SampleID)) + # define dataframe and paramters
  ...
  scale_fill_manual(values = c("white",meisColor, meisColor, pbxColor, pbxColor, "white",meisColor, meisColor, pbxColor, pbxColor, "white",meisColor, meisColor, pbxColor, pbxColor)) # call fill colors
...
ggplot(filter(TailS, Timepoint == "72hpa"), aes(x=SampleID, y=Measurement, fill=SampleID)) +
  ...
  xlab("") +
   stat_compare_means(comparisons = comparisons, method = "t.test", label.y = c(750, 825, 900, 975), label = "p.signif") +
    Seurat:::RotatedAxis() +
  scale_fill_manual(values = c("white",meisColor, meisColor, pbxColor, pbxColor, "white",meisColor, meisColor, pbxColor, pbxColor, "white",meisColor, meisColor, pbxColor, pbxColor))
